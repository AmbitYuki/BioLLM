...
representations = [["name", "SMILES", "inchi", "selfies"], ["SMILES", "inchi", "selfies"]]
...
def train_test_model(num_classes, representation, num_train_points, seed):
    ...
    data['binned'] = pd.qcut(data['E isomer pi-pi* wavelength in nm'], num_classes, labels=np.arange(num_classes))
    ...
    cm_confident = evaluate_classification(test['binned'].values[confident], predictions_mode[confident])
        ...
        for num_class in num_classes:
            for num_train_points in num_training_points:
                for representation in representations:
                    try:
                        train_test_model(num_class, representation, num_train_points, i)
